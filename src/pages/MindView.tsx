...
export default function MindView() {
  ...
  const tools = useMemo(() => [
    {
      to: "/app/checkin", title: T("Check-in روزانه", "Daily Check-in"), desc: T("ثبت خلق، انرژی، تمرکز و خواب", "Log mood, energy, focus & sleep"),
      icon: Activity, gradient: "from-rose-500 via-pink-500 to-fuchsia-500",
    },
    {
      to: "/app/thoughts", title: T("ثبت افکار CBT", "CBT Thought Log"), desc: T("بازسازی شناختی و کشف خطاهای فکری", "Cognitive reframing & distortion discovery"),
      icon: BookOpen, gradient: "from-violet-500 via-purple-500 to-indigo-500",
    },
    {
      to: "/app/abc", title: T("مدل ABC", "ABC Model"), desc: T("محرک، باور، نتیجه — کشف الگوهای رفتاری", "Activating event, Belief, Consequence — behavioral patterns"),
      icon: Zap, gradient: "from-amber-500 via-orange-500 to-red-500",
    },
    {
      to: "/app/worry", title: T("حل نگرانی و مسئله", "Worry & Problem Solving"), desc: T("تفکیک نگرانی قابل‌حل از غیرقابل‌حل", "Differentiate solvable from unsolvable worries"),
      icon: Wind, gradient: "from-sky-500 via-blue-500 to-indigo-500",
    },
    {
      to: "/app/values", title: T("ارزش‌ها و اهداف (ACT)", "Values & Goals (ACT)"), desc: T("ارزش‌ها، شکاف‌ها و هدف‌های معنادار", "Core values, gaps & meaningful goals"),
      icon: Compass, gradient: "from-emerald-500 via-teal-500 to-cyan-500",
    },
    {
      to: "/app/breathing", title: T("تمرین تنفس ۳بعدی", "3D Breathing Practice"), desc: T("۵ الگوی تنفس با راهنمای بصری ۳ بعدی", "5 breathing patterns with 3D visual guide"),
      icon: Wind, gradient: "from-teal-500 via-cyan-500 to-sky-500",
    },
    {
      to: "/app/socratic", title: T("چت سقراطی", "Socratic Chat"), desc: T("گفت‌وگوی هدایت‌شده با AI برای چالش افکار", "AI guided dialogue to challenge thoughts"),
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 md:p-8 space-y-6 pb-20 animate-fade-in" dir={isEn ? "ltr" : "rtl"}>
      {/* Hero */}
      <Card className="p-6 md:p-8 border-border/70 bg-card/60 backdrop-blur-xs shadow-xs">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-2xl md:text-3xl font-bold mb-1">{T("ذهن آرام، تصمیم بهتر", "Calm Mind, Better Decisions")}</h1>
            <p className="text-sm text-muted-foreground max-w-xl">
              {T("یک نگاه به وضعیت امروز، روند ۳۰ روز، الگوهای فکری و ابزارهای CBT.", "An overview of today's status, 30-day trends, thought patterns, and CBT tools.")}
            </p>
            {isToday && today && (
              <div className="mt-4 flex items-center gap-4 text-sm text-muted-foreground flex-wrap">
                <span className="flex items-center gap-1.5"><Activity className="w-4 h-4 text-rose-500" /> {T("خلق", "Mood")} {today.mood ?? "—"}</span>
                <span className="flex items-center gap-1.5"><Zap className="w-4 h-4 text-amber-500" /> {T("انرژی", "Energy")} {today.energy ?? "—"}</span>
                <span className="flex items-center gap-1.5"><Compass className="w-4 h-4 text-sky-500" /> {T("تمرکز", "Focus")} {today.focus ?? "—"}</span>
              </div>
            )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard label={T("ابزارهای ذهن", "Mind Tools")} value={tools.length} icon={Brain} tone="text-emerald-500" />
      </div>
      ...
      <Card className="p-5 border-border/60 bg-card/60 shadow-sm">
        <h3 className="font-semibold mb-4 flex items-center gap-2 text-foreground">
          <ClipboardCheck className="w-4 h-4 text-primary" /> {T("غربالگرهای کوتاه", "Brief Screeners")}
          <span className="text-xs font-normal text-muted-foreground">{T("— ردیابی شخصی، نه تشخیص بالینی", "— Personal tracking, not clinical diagnosis")}</span>
        </h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            return (
              <Link key={type} to={`/app/screener/${type}`}
                ...
                {last ? (
                  ...
                  <div className="text-[11px] text-muted-foreground mt-2">{T("شروع تست", "Start test")}</div>
                )}
              </Link>
            );
          })}
      ...
      {/* Tools grid */}
      <Card className="p-5 border-border/60 bg-card/60 shadow-sm">
        <h3 className="font-semibold mb-4 flex items-center gap-2 text-foreground"><Brain className="w-4 h-4 text-primary" /> {T("ابزارهای ذهن", "Mind Tools")}</h3>
