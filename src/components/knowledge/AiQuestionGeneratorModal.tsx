...
export const AiQuestionGeneratorModal: React.FC<AiQuestionGeneratorModalProps> = ({
  ...
  // Sync initialText when modal opens, and run auto-generation once per source.
  useEffect(() => {
    if (!open) {
    ...
    const sourceKey = `${documentTitle || ""}\u0000${initialText || ""}`;
    ...
    if (
      initialText && initialText.trim().length > 15 && candidateCards.length === 0 &&
      autoGeneratedSourceRef.current !== sourceKey
    ) {
      autoGeneratedSourceRef.current = sourceKey;
      void handleGenerate(initialText);
    }
  }, [open, initialText, documentTitle, candidateCards.length, handleGenerate]);
