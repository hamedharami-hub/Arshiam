...
public abstract class ActionHubWidgetProvider extends AppWidgetProvider {
    abstract String title(); abstract String subtitle(); abstract String primaryLabel(); abstract String primaryRoute();
    ...
    @Override public void onUpdate(Context c, AppWidgetManager manager, int[] ids) {
        for (int id : ids) {
            ...
            for (int i = 0; i < buttons.length; i++) {
                ...
                PendingIntent action = "quick-add".equals(routes[i]) ? AgendaWidgetProvider.quickCreate(c, id * 10 + i + 2)
